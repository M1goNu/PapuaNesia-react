...
function AboutUs({theme}) {
  ...
  return (
    <div className={`mx-auto p-4 pt-6 md:p-6 lg:p-12 ${theme === 'light' ? 'text-customRed' : 'text-costumBlue'}`}>
        ...
        <div key={index} className="flex flex-wrap -mx-4 justify-center items-center mb-8">
          {row.map((member) => (
            <div key={member.name} className="w-full md:w-1/2 lg:w-1/3 p-4">
              <div className={`flex flex-col items-center rounded-lg shadow-lg overflow-hidden ${theme === 'light' ? 'bg-customRed text-customWhite' : 'bg-costumBlue text-customWhite'}`}>
